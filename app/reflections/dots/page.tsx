    ...
    <>
      ...
      <div style={{background:'var(--black)'}}>
        <div className="hero-ratio" style={{position:'relative',width:'100%',aspectRatio:'16/7'}}>
          ...
          <div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <h1 style={{fontSize:'clamp(24px,4vw,56px)',fontWeight:600,color:'#fff',letterSpacing:'-.03em',textAlign:'center',padding:'0 24px'}}>&ldquo;Dots to Decisions&rdquo;</h1>
          </div>
        </div>
      </div>
      ...
      <div className="r-pad" style={{padding:'96px 8%',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div className="r-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:56,alignItems:'start'}}>
          <div>
            <h2 style={{fontSize:'clamp(24px,3.5vw,44px)',fontWeight:600,letterSpacing:'-.03em',lineHeight:1.15,marginBottom:20}}>Research without synthesis is just noise with a spreadsheet.</h2>
            ...
            <p style={{fontSize:15,color:'var(--muted)',lineHeight:1.85,fontWeight:600}}>Synthesis is the skill most designers never talk about — and the one that determines whether research actually changes anything.</p>
          </div>
          <div>
            <QuoteBlock text="Collecting data is the easy part. The hard part is sitting with it long enough to hear what it is actually saying." cite="What 100+ Reddit threads taught me" />
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              {[
                {num:'01',title:'Data without direction',desc:'More research does not mean more clarity. Without a synthesis process, more data creates more confusion.'},
                {num:'02',title:'Patterns hiding in plain sight',desc:'The most important insights are rarely in what users say directly. They are in what keeps coming up across sources.'},
      ...
      <div className="r-pad" style={{padding:'96px 8%',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div className="r-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:56,alignItems:'start'}}>
          <div>
            <h2 style={{fontSize:'clamp(22px,3vw,40px)',fontWeight:600,letterSpacing:'-.03em',lineHeight:1.15,marginBottom:20}}>From raw data to a direction worth trusting.</h2>
            <p style={{fontSize:15,color:'var(--muted)',lineHeight:1.85,marginBottom:16}}>When I worked on Lighthouse AI, I ran 50 surveys, 20 interviews, and dug through over 100 Reddit threads. The data was rich — but also overwhelming, contradictory, and scattered.</p>
            <p style={{fontSize:15,color:'var(--muted)',lineHeight:1.85,marginBottom:24}}>Synthesis is not about organizing data. It is about finding the thread that runs through all of it.</p>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:12}}>
            {[
              {step:'01',label:'Collect without filtering',desc:'Pull everything in first. Do not decide what matters yet.'},
              ...
              {step:'03',label:'Find the emotion underneath',desc:'Vague posts and frustrated language reveal overwhelm — the real signal.'},
              {step:'04',label:'Name the insight, not the data',desc:'Stop at patterns, not quotes. The insight is the conclusion.'},
              {step:'05',label:'Connect insight to decision',desc:'Every synthesis should end with a direction. If it does not change something, it was not synthesis.'},
            ].map(({step,label,desc})=>(
              <div key={step} style={{display:'flex',gap:16,alignItems:'flex-start',padding:'14px 18px',borderRadius:12,background:'var(--gray)',border:'1px solid var(--border)'}}>
                ...
                <div><div style={{fontSize:14,fontWeight:600,marginBottom:4}}>{label}</div><p style={{fontSize:13,color:'var(--muted)',lineHeight:1.5}}>{desc}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="r-pad" style={{padding:'96px 8%',maxWidth:1200,margin:'0 auto'}}>
        ...
        <div className="r-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:56,alignItems:'start'}}>
          ...
          <div>
            <h2 style={{fontSize:'clamp(22px,3vw,40px)',fontWeight:600,letterSpacing:'-.03em',lineHeight:1.15,marginBottom:20}}>The insight is never what you expect.</h2>
            ...
            <p style={{fontSize:15,color:'var(--muted)',lineHeight:1.85,marginBottom:24}}>That single shift — from information product to emotional support product — changed the entire design direction.</p>
            <QuoteBlock text="Vague posts do not mean unclear thinking. They often mean the person does not yet have language for what they are feeling. That is exactly where the product needs to meet them." cite="The Reddit insight that changed Lighthouse AI" />
          </div>
        </div>
      </div>
      ...
      <div style={{background:'var(--black)',padding:'80px 8%'}}>
        <div style={{maxWidth:1200,margin:'0 auto'}}>
          ...
          <p style={{fontSize:15,color:'#666',lineHeight:1.85,maxWidth:580,marginBottom:48}}>Research without synthesis is just documentation. The difference is what happens after the data is collected.</p>
          <div className="r-grid-3" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:48}}>
            <InsightCard num="01" title="Patterns over quotes" body="A pattern that shows up across 50 people is actually useful. Quotes illustrate. Patterns decide." />
            <InsightCard num="02" title="Vague signals are the loudest" body="Frustrated, vague, or emotional responses are the clearest signal that something is genuinely wrong at the emotional level." />
            <InsightCard num="03" title="Synthesis ends in a decision" body="If your synthesis process produces more questions without a direction, it is not done yet." />
          </div>
          <div style={{borderTop:'1px solid #1d1d1d',paddingTop:48}}>
            ...
            <div className="r-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:32,alignItems:'center'}}>
              <div>
                <h3 style={{fontSize:'clamp(20px,2.5vw,34px)',fontWeight:600,letterSpacing:'-.03em',lineHeight:1.2,color:'#fff',marginBottom:16}}>Connecting dots is the work. Everything else is preparation.</h3>
                <p style={{fontSize:15,color:'#666',lineHeight:1.85,marginBottom:16}}>Collecting research is the easy part. Synthesis is judgment — deciding what the data is actually saying and what to do about it.</p>
                <p style={{fontSize:15,color:'#555',lineHeight:1.85}}>That skill comes from sitting with the mess long enough to see the shape of what it is trying to say.</p>
              </div>
      ...
      <div style={{background:'var(--black)',padding:'80px 8%',borderTop:'1px solid #1a1a1a',textAlign:'center'}}>
        ...
        <div style={{fontSize:'clamp(28px,5vw,60px)',fontWeight:600,color:'#fff',letterSpacing:'-.04em',marginBottom:8,lineHeight:1}}>Good Design<br />is Good Business</div>
